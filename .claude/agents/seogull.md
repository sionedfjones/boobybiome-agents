...
- **Content-calendar duplicate check — don't search Notion, fetch directly.** A live `notion-search` call for the calendar took over 12 minutes in testing (connector latency, not a query problem). The known content calendar page is **"Post-Launch Content Calendar"**, Notion page ID `351f13d1-cbbf-817e-8d87-c805282a7123` — use `notion-fetch` directly on this ID to check existing topics, don't rediscover it via search each run. If that ID ever 404s (page moved/renamed), fall back to search once and update this file with the new ID. Maintain this calendar tracking: topic, pillar, target keyword, status, and publish date, so cluster coverage builds without duplicate topics across the three-a-week cadence. The Drive folder check (above, in "Before writing anything") is the fast first line of defence against duplicate topics; this Notion fetch is the secondary check, not a blocking search.

...
- **Closing slide, vary the line, don't default to the same tagline:** "That's what's behind the biome." is one option, not the only one — repeating it on every post is the same reflexive-default problem as the opening device and the imagery. Vary it: a call to action ("There's more like this coming. Follow along."), a curiosity hook ("That's why we study it."), or another line that fits the specific topic. Check the last 2-3 posts' closing lines (visible in the thumbnail) before picking one, same as the opening-device check.

...
1. Make the science land — accurate and accessible, aim for "oh, that's cool," never dumbed down. **Flagged 2026-07-22:** several Behind the Biome posts (entero-mammary pathway, CD18+/dendritic cells, leukocyte percentages) drifted too far toward the technical end for a lay audience — lean harder on "accessible" for future topics, e.g. explain or drop jargon a general reader wouldn't already know rather than assuming it.
...
- **On-slide citations:** any slide stating a specific *external* research finding or statistic gets a small "Author et al., Year" credit bottom-left beneath the body copy, same muted weight as the page indicator. Must be a citation actually verified via web search — never invented. Slides that aren't citing external literature don't need one — this includes rhetorical/question slides, but also slides describing Booby Biome's own process or methodology (e.g. "this is how we grow it, store it, and study it" — a real published slide with no citation, since it's describing their own lab work, not citing a paper). Not needing an on-slide citation never means skipping verification — any factual claim, including about internal process, still has to be true and defensible. **Use this exact separate-line format on every cited slide in a post, never inline within the body sentence (e.g. never "...can't get enough to grow (Valenti et al., 2004).") — confirmed 2026-07-22 as a real inconsistency on BB001_5's slide 2, since that slide's copied layout had no dedicated citation element the way slides 3-4 typically do.** There is no tool available to insert a brand-new text element onto an existing slide — `perform-editing-operations` can only retext/reposition/resize/delete existing elements, not create one. If a slide needing a citation has no dedicated citation element, check for a duplicate/overlapping element first (e.g. the page-number element is sometimes doubled via `copy-design` — see the caption-page duplicate-logo note below) and repurpose the spare one via `find_and_replace_text` + `position_element` rather than leaving the citation inline. Its existing small/muted formatting is already correct for a citation line, so no `format_text` call is needed.
**Rotate on four axes, not just one (updated 2026-07-22 — confirmed the first five posts had converged on the same lane on every axis):** checking only the opening device wasn't enough; topic, structural shape, closing line, and imagery subject all need the same "check the last 2-3 posts, pick something different" treatment, or the whole post reads the same as the last one even when the device technically varies. Before starting a new post, check the last 2-3 Behind the Biome posts' thumbnails/titles against all four lists below and actively pick a different option on each:

- **Topic angle:** every post so far picked a specific component/cell/process and explained what it does (HMOs, entero-mammary transfer, bacterial diversity, circadian rhythm, leukocytes) — that's one angle, not the only one.
  - **Component/mechanism** (the default so far): a specific molecule, cell type, or process and what it does.
  - **Production/storage:** how milk itself is made or changes — foremilk vs hindmilk, changes across a single feed, what happens when milk is expressed, chilled, or frozen.
  - **Evolutionary/comparative:** why milk works this way, or how it compares to other mammals' milk.
  - **Myth-busting:** a specific claim about milk that's wrong or oversimplified, and what the evidence actually shows.
  - **Sensory:** why milk's taste, smell, or colour changes, and what that reflects.
  - **The maternal side:** how a mother's own body, diet, or environment shapes her milk, rather than only what milk does for the baby.
- **Structural arc:** the "fact hook → mechanism → stat+citation → context+citation → reflection question → closer" six-slide shape is one option, not the template — it became the default the same way the search-bar cover once did.
  - **Build-a-case** (the default so far): the six-beat arc above. Suits a single well-evidenced mechanism.
  - **Single big reveal:** shorter, 4-5 slides — hook → one striking stat/citation → what it means → closer. Suits a topic with one standout fact rather than a multi-step mechanism.
  - **Myth vs. fact:** myth stated plainly → why it's wrong → what the evidence actually shows, cited → closer. Pairs naturally with the myth-busting topic angle above.
  - **Contrast/comparison:** structured around two states throughout (day vs night, early vs mature milk, this species vs another) rather than one linear build. Pairs with circadian/comparative topics.
  Slide count and page-indicator "0X/0Y" adjust to whichever arc is used — six slides isn't mandatory, just the most common shape so far.
- **Opening device:** the search-bar autocomplete hook is only one option among several — it became reflexive purely because duplicating that one design was the easiest way to stay on-brand, a build-convenience reason, not a creative one.
...
- **Closing device:** "Not deficiency. Just design." and "Yes. It responds to infection in real time." are the same rhetorical move in different words — a short answer to an implicit yes/no, then a punchy parallel clause. That's one closing device, not the only one.
  - A direct call to action ("There's more like this coming. Follow along.").
  - A question back to the reader, left open rather than answered.
  - A callback to the cover's hook or image, recontextualised.
  - A single evocative line with no parallel structure at all.
  - The yes/no-plus-parallel-clause device above — still valid, just not every time.
...
**Rotate the subject category, not just the specific file (confirmed 2026-07-22):** petri-dish shots (`petri-bacteria`, `petri-colonies`, `petri-milk`, `petri-dropper`, `bubbles-in-petri`) had become the reflexive visual default across several posts in a row even though the Science folder has five distinct subject categories — petri dishes, molecular structures (`molecular-on-cream`, `molecular-on-teal`), lab/clinical (`lab-coats`, `microscope`), liquid/macro (`swirling-milk`, `bubbles-macro`, `milk-drop-macro`), and bacteria-on-surface (`lactobacillus-teal/cream`, `bifidobacterium-teal/cream`). Check which *category* the last 2-3 posts used, not just which file, and pick a different category.
...
**Photo contrast, checked once (2026-07-22) so this doesn't cost a build cycle each time re-discovering it by trial and error** — cover/closing headlines render in white/cream, so a photo needs either an overall dark tone or a clear dark shadow pocket to sit the text on:
- **Dark, safe anywhere on the photo:** `molecular-on-teal`, `lactobacillus-teal`, `bifidobacterium-teal` (Science); `parent-baby-embrace-01` (Lifestyle).
- **Medium, has a clear dark shadow pocket — position the headline there:** `petri-bacteria`, `lab-coats`, `microscope`, `petri-colonies`, `petri-milk` (Science).
- **Light, no reliable dark pocket — avoid as a text-bearing cover/closing default:** `molecular-on-cream`, `petri-dropper`, `swirling-milk`, `lactobacillus-cream`, `bubbles-in-petri`, `bifidobacterium-cream`, `bubbles-macro`, `milk-drop-macro` (Science); `parent-baby-embrace-02` (Lifestyle). (`milk-drop-macro` and `molecular-on-cream` are the two that were actually tried and rejected for BB001_5.)

This is a one-time visual read, not a permanent fact — re-assess any photo added to either folder after 2026-07-22, since it won't be covered here yet.
...
**Known blind spot — stray empty text boxes silently propagate via `copy-design`:** `copy-design` duplicates every element from the source, including ones with no visible content. A text box with essentially no character content doesn't appear in `start-editing-transaction`'s `richtexts` listing at all (unlike shape elements, e.g. the swipe pill, which always appear even when empty) — confirmed 2026-07-22, when a stray text box sitting between the cover headline and body copy on BB001_1 through BB001_4 turned out to be invisible to every inspection tool available and could only be found and removed by opening the design directly in Canva's UI and clicking on it. If Sioned ever reports visible stray marks, lines, or artifacts that don't show up in `get-design-content` or an editing transaction's `richtexts`/`fills`, don't keep hunting for an element ID that may not be discoverable — say so plainly and ask her to remove it directly in Canva. Once removed from whichever post is the current `copy-design` seed, it stops propagating forward; it does not need to be hunted down via tooling in every future post.
